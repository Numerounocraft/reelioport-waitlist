...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- SSR-safe mount guard, no external system to subscribe to
    setIsMounted(true);
  }, []);

  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- derives sphere layout from images prop once mounted
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      <div
        ...
        style={{ animation: "img-sphere-fade-in 0.3s ease-out" }}
      >
        <div
          className="w-full max-w-md overflow-hidden rounded-xl bg-white"
          onClick={(e) => e.stopPropagation()}
          style={{ animation: "img-sphere-scale-in 0.3s ease-out" }}
        >
          <div className="relative aspect-square">
            <img src={selectedImage.src} alt={selectedImage.alt} className="h-full w-full object-cover" />
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 right-2 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-black/50 text-white transition-all hover:bg-black/70"
            >
              <X size={16} />
            </button>
          </div>
  ...
  return (
    <>
      <style>{`
        @keyframes img-sphere-fade-in {
          from { opacity: 0; }
          to { opacity: 1; }
        }
        @keyframes img-sphere-scale-in {
